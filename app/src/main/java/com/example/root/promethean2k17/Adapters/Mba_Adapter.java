...
public class Mba_Adapter extends RecyclerView.Adapter<Mba_Adapter.MbaViewHolder> {
    ...
    @Override
    public void onBindViewHolder(Mba_Adapter.MbaViewHolder holder, int position) {
        Mba_Model model=arraylist.get(position);
        Glide.with(context).load(model.getMba_pic()).diskCacheStrategy(DiskCacheStrategy.SOURCE).error(R.drawable.ic_error).into(holder.Mba_pic);
    ...
    public static class MbaViewHolder extends RecyclerView.ViewHolder{
        ...
        public MbaViewHolder(View itemView) {
            super(itemView);
            Mba_pic = (ImageView) itemView.findViewById(R.id.home_imgview);
